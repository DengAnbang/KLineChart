...
public class CCIDraw implements IChartDraw<ICCI> {
    ...
    @Override
    public void drawTranslated(@Nullable ICCI lastPoint, @NonNull ICCI curPoint, float lastX, float curX, @NonNull Canvas canvas, @NonNull BaseKLineChartView view, int position, boolean isNewest, float animationPrice) {
        // do nothing.
    }

    @Override
    public void drawIndicatorLine(@NonNull ICCI point, @NonNull Canvas canvas, @NonNull BaseKLineChartView view, float x, boolean isFirst, boolean isLast) {
        if (isFirst) {
        ...
        List<MAIndicator> items = point.getCCI();
        if (KChartListUtils.isEmpty(point.getCCI())) {
            return;
        }
        for (int i = 0; i < items.size(); i++) {
            ...
            IndicatorLine line = getCacheIndicatorLine(i, KChartConstant.Sub.CCI);
            float y = view.getChildY(KChartConstant.Sub.CCI, (float) ma.getCalcResult());
            if (line.isFirst()) {
    ...
    public void drawText(@NonNull Canvas canvas, @NonNull BaseKLineChartView view, int position, float x, float y) {
        ...
        drawValueText(point.getCCI(), canvas, view, x, y);
    }
    ...
    private void drawValueText(List<MAIndicator> items, Canvas canvas,
        ...
        String displayText = CalcParamsUtil.getCalcParamsDisplayText(
                KChartConstant.Sub.CCI,
                String.valueOf(cciPeriod)
        );
